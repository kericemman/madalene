...
function Modal({ title, eyebrow, onClose, children }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6">
      ...
      <section role="dialog" aria-modal="true" aria-label={title} className="relative flex max-h-[calc(100dvh-1rem)] w-full max-w-4xl flex-col overflow-hidden rounded border border-sage bg-mistWhite shadow-[0_24px_70px_rgba(26,26,26,0.32)]">
  ...
  return (
    <section>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[["Live version", activeVersion ? `v${activeVersion.version}` : "None"], ["Questions in view", selectedQuestions.length], ["Result stages", scoreRanges.length], ["Active rules", rules.filter((rule) => rule.active).length]].map(([label, value]) => <article key={label} className="rounded border border-sage bg-white p-5 shadow-[0_12px_28px_rgba(26,26,26,0.035)]"><BarChart3 className="text-deepEmerald" size={22} aria-hidden="true" /><p className="mt-4 text-xs font-extrabold uppercase tracking-[0.14em] text-charcoal/48">{label}</p><p className="mt-1 text-2xl font-extrabold text-charcoal">{value}</p></article>)}
      </div>
